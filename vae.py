...
class VAE(nn.Module):
    ...
    def get_loss(self, x):
        ...
        reconstruction_loss = torch.mean((x - recons) ** 2)
        var = torch.exp(log_var)
        divergence_loss = var + (mu ** 2) - 1 - log_var
        divergence_loss = torch.sum(divergence_loss / 2, dim = 1)
        divergence_loss = torch.mean(divergence_loss)
        divergence_loss = reconstruction_loss + divergence_loss
